...
To develop our algorithm we could make another peak detector, after the thorough inspection of the available ECG signals and some trials and errors, we arrived to the conclusion that the energy of the error signal between two peaks is more accurate to detect an anomaly in that specific region. This is because electrocardiogram failure is more of a distributed problem over the whole pseudo-period, rather than a localized event.\par

The designed algorithm calculates the energy of each pseudo-period of the error signal. If said energy is sufficiently high (higher than a calibrated value reached by trial and error) the algorithm reports an anomaly. A weighted average was used to assign an epicenter to the anomaly, thus being able to compare our algorithm with the real anomalies, taken from the annotations of the MIT-BIH database .
...
The algorithm time detections should now be compared to the times the real heart anomalies were detected. This timing information was taken from the database annotations. \par
...
\label{fig}
\end{figure}
The comparison module from the previous diagram measures the performance of the algorithm. Its inputs are the detected anomalies and the real anomalies points.\par
Ideally, based on the measured performance, the other 4 blocks of the global system (highpassH, peakDetector, ARF, anommalyDetector) would be changed and improved continually in order to enhance the system.\par

Two different kinds of accuracy tests were done:
First, we calculated from all detected positive the amount that were calculated correctly, and then we calculated from all the real positives, how much were correctly detected. Note that two points of view are similar but complementary. 
...
\bibitem{b3} https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1232857/
\end{thebibliography}
\end{document}
